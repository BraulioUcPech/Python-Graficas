...
game = PongEnvironment(max_life=3, movimiento_px = 3)
state = game.reset()
reward, done = None, None
itera=0
while (done != True) and (itera < 3000 and game.total_reward<=1000):
    old_state = np.array(state)
    next_action = learner.get_next_step(state, game)
    state, reward, done = game.step(next_action, animate=True)
    itera+=1
print("Partidas[", itera, "] Puntos[", game.total_reward,"]")
print("Fin del Juego!")

learner, game = play(rounds=5000, discount_factor = 0.2, learning_rate = 0.1, ratio_explotacion=0.85, learner=learner, game=game)
